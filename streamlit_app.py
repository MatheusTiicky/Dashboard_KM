                ...
                if taxa_cancelamento <= meta_taxa:
                    ...
                    st.markdown(
                        ...
                        <div style="text-align:center; margin-top:10px; font-size:20px; font-weight:bold;">
                            🚨 <span class="tarja-blink" style="color:#ef4444;">Status: ACIMA DA META</span>
                        </div>
    ...
    with tab2:
        ...
        if df_tab2.empty:
            ...
            if len(df_trend_medias_temporal) >= 2:
                ...
                # Abreviações fixas em PT-BR (não dependem de locale)
                meses_abrev = {
                    1: "Jan",  2: "Fev",  3: "Mar",
                    4: "Abr",  5: "Mai",  6: "Jun",
                    7: "Jul",  8: "Ago",  9: "Set",
                ...
                fig_trend_medias_temporal.update_xaxes(
                    tickvals=tickvals,
                    ticktext=[meses_abrev[d.month] + "/" + d.strftime("%Y") for d in tickvals]
                )
    ...
    with tab3:
        ...
        if len(usuarios_disponiveis_tab3) < 2:
            ...
            if usuario_a and usuario_b:
                ...
                col1, col_center, col2 = st.columns(3)
                ...
                with col1:
                    ...
                    """, unsafe_allow_html=True)
                with col_center:
                    # Calcular a média mensal geral do período para ambos os usuários
                    # Para isso, vamos considerar os dados de ambos os usuários no período selecionado
                    # e calcular a média mensal de emissões.
                    
                    # Combinar os dados de ambos os usuários para o cálculo da média geral
                    dados_combinados = pd.concat([dados_a, dados_b])
                    
                    if not dados_combinados.empty:
                        # Calcular a emissão geral total
                        emissao_geral_total = dados_combinados["CTRC_EMITIDO"].sum()

                        # Calcular a média diária do período combinado
                        emissoes_diarias_combinadas = dados_combinados.groupby(dados_combinados["DATA_EMISSÃO"].dt.date)["CTRC_EMITIDO"].sum()
                        media_diaria_geral = emissoes_diarias_combinadas.mean()

                        # Calcular a média mensal do período combinado
                        emissoes_mensais_combinadas = dados_combinados.groupby(dados_combinados["DATA_EMISSÃO"].dt.to_period("M"))["CTRC_EMITIDO"].sum()
                        media_mensal_geral = emissoes_mensais_combinadas.mean()
                    else:
                        emissao_geral_total = 0
                    ...
                    <div class="kpi-card kpi-orange">
                        <div class="kpi-icon">📦</div>
                        <div class="kpi-value">{format_number(emissao_geral_total)}</div>
                        <div class="kpi-label">Emissão Geral Total</div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    <div class="kpi-card kpi-orange">
                        <div class="kpi-icon">📅</div>
                        <div class="kpi-value">{format_number(media_diaria_geral)}</div>
                        <div class="kpi-label">Média Diária Geral Total</div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    <div class="kpi-card kpi-orange">
                        <div class="kpi-icon">🗓️</div>
                        <div class="kpi-value">{format_number(media_mensal_geral)}</div>
                        <div class="kpi-label">Média Mensal Geral Total</div>
                    </div>
                    """, unsafe_allow_html=True)
                with col2:
                    st.markdown(f"""
                ...
                diferenca_media_diaria = abs(media_diaria_a - media_diaria_b)
                diferenca_media_mensal = abs(media_mensal_a - media_mensal_b)                
                # Determinar qual usuário tem mais emissões
                usuario_maior_total = usuario_a if total_a > total_b else usuario_b
                usuario_maior_media = usuario_a if media_diaria_a > media_diaria_b else usuario_b

                # Obter o nome do mês atual para os insights
                nome_mes_atual_insight = mes_selecionado if mes_selecionado != 'Todos' else 'o período selecionado'
                ...
                st.markdown(f"""
                    <div style="
                        ...
                        ">                        <div style="font-size: 1.1rem; font-weight: 700; margin-bottom: 0.5rem; display:flex; align-items:center; gap:0.5rem;">📦 Total</div>
                        Em {nome_mes_atual_insight}, {usuario_maior_total} emitiu {format_number(diferenca_total)} CTRCs a mais.
                    </div>
                    """, unsafe_allow_html=True)
                ...
                st.markdown(f"""
                    <div style="
                        ...
                        font-family: 'Inter', sans-serif;
                        ">
                        <div style="font-size: 1.1rem; font-weight: 700; margin-bottom: 0.5rem; display:flex; align-items:center; gap:0.5rem;">📅 Média Diária</div>
                        Em {nome_mes_atual_insight}, {usuario_maior_media} apresentou uma média diária de {format_number(diferenca_media_diaria)} CTRCs a mais.
                    </div>
                    """, unsafe_allow_html=True)
                ...
                st.markdown(f"""
                    <div style="
                        ...
                        font-family: 'Inter', sans-serif;
                        ">
                        <div style="font-size: 1.1rem; font-weight: 700; margin-bottom: 0.5rem; display:flex; align-items:center; gap:0.5rem;">🗓️ Média Mensal</div>
                        Em {nome_mes_atual_insight}, {usuario_maior_media} apresentou uma média mensal de {format_number(diferenca_media_mensal)} CTRCs a mais.
                    </div>
                    """, unsafe_allow_html=True)        
